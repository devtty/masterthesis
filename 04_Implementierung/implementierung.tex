...
\section{Lernmodus}
Der Lernmodus erfasst alle HTTP-Anfragen und speichert diese in einer eigenen Text-Datei. Bei aktiviertem Lernmodus geht WebCastellum davon aus, dass alle Anfragen auch gewünschte Anfragen sind. Aus den aufgezeichneten Anfragen lässt sich beispielsweise eine Whitelist an URL-Pfaden oder Stichwörtern generieren, die für eine weitere Absicherung nützlich sind. Technisch wird an dieser Stelle ebenfalls das Logging-Framework genutzt und eine Erweiterung kann in Anlehnung an Abbildung~\ref{fig.imversand} erfolgen. Der Nachrichtenversand erfolgt auch in diesem Fall an die zentrale Komponente, wo beispielsweise auch eine weitere Verarbeitung stattfinden kann.
