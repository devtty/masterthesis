...
\textcolor{bhtGray}{\ding{110} Kozik über den Datensatz CSIC2010~\cite{kozik2019}} For instance, the original data set currently contains mainly requests that are complied with \glq\verb=application/x-www-form-urlencoded=\grq encoding standard. On the other hand, the modern web applications are currently also using different types of request content, e.g. \glq\verb=application/json=\grq, \glq\verb=text/x-gwt-rpc=\grq or other proprietary types.\\

In ihrer Arbeit testeten sie gegen eine GWT\footnote{Google Web Toolkit}-basierte Anwendung und erweiterten für ihre Zwecke den CSIC2010-Datensatz um 1500 \emph{normale} Anfragen und 2000 Angriffe.
