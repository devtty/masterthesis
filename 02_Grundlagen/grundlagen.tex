...
Bei dieser \emph{Sicherheitsregel} wird die \emph{aufgerufene Adresse} (\texttt{REQUEST_URI VARIABLE}) auf \emph{Übereinstimmung mit einem regulären Ausdruck} (\texttt{.ht} \texttt{OPERATOR}) geprüft und bei Übereinstimmung die Ausführung \emph{verweigert} (\texttt{deny} \texttt{ACTIONS}).
